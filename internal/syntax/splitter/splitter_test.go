...
func TestSplitterWithValidSpaces(t *testing.T) {
	sql := "SELECT 'g.Year','g.Industry_aggregation_NZSIOC' FROM path:../../../testdata/example.csv AS g WHERE 'g.Area' = 'A100100'"
	s := NewSplitter(sql)
...
func TestSplitterWithInValidSpaces(t *testing.T) {
	sql := "SELECT     'g.Year'     FROM     path:testdata/example.csv      WHERE          'g.Area' =            'A100100'"
	s := NewSplitter(sql)
